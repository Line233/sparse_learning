...
import sys
sys.path.append('/root/LK/sparse_learning/')
...
def setup_logger(args):
    ...
    log_path = './logs/{0}_{1}_{2}.log'.format(args.model, args.density, hashlib.md5(
        str(args_copy).encode('utf-8')).hexdigest()[:8])
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if batch_idx % args.log_interval == 0:
            print_and_log('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
                epoch, batch_idx *
                len(data), len(train_loader)*args.batch_size,
...
def main():
    ...
    parser.add_argument('--save-model', type=str,
                        default='./models/model.pt', help='For Saving the current Model')
    ...
    for i in range(args.iters):
        ...
        if args.model not in models:
            ...
            model = cls(
                *(cls_args + [args.save_features, args.bench])).to(device)
            print_and_log(model)
            print_and_log('='*60)
        ...
        if args.optimizer == 'sgd':
            ...
            ), lr=args.lr, momentum=args.momentum, weight_decay=args.l2, nesterov=True)
        ...
        lr_scheduler = optim.lr_scheduler.StepLR(
            optimizer, args.decay_frequency, gamma=0.1)
        ...
        if not args.dense:
            if args.decay_schedule == 'cosine':
                decay = CosineDecay(args.prune_rate, len(
                    train_loader)*(args.epochs))
            elif args.decay_schedule == 'linear':
                decay = LinearDecay(args.prune_rate, len(
                    train_loader)*(args.epochs))
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            train(args, model, device, train_loader,
                  optimizer, epoch, lr_scheduler, mask)
